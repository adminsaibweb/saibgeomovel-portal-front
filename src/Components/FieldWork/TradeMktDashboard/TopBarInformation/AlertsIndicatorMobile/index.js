...
export default class ClientsIndicatorMobile extends Component {
  ...
  handleAlerts = async () => {
    const { alerts, typeFilter } = this.props;
    this.setState({
      countAlerts: alerts.filter((alert) => parseInt(alert.typeColaborator) === parseInt(typeFilter)).length,
      countAlertsReaded: alerts.filter(
        (alert) => parseInt(alert.typeColaborator) === parseInt(typeFilter) && alert.status === 1
      ).length,
      countAlertsPeding: alerts.filter(
        (alert) => parseInt(alert.typeColaborator) === parseInt(typeFilter) && alert.status === 0
      ).length,
    });
  };
