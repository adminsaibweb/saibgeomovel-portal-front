...
import { MarketingIndicators } from '../../../Components/ResultsAnalysis/MarketingDashboard/MarketingIndicators';
import { VouchersIndicators } from '../../../Components/ResultsAnalysis/MarketingDashboard/VouchersIndicators';
import { OffersIndicators } from '../../../Components/ResultsAnalysis/MarketingDashboard/OffersIndicators';
import { CoastCenterIndicators } from '../../../Components/ResultsAnalysis/MarketingDashboard/CoastCenterIndicators';
...
class MarketingDashboard extends Component {
  ...
  componentDidMount = async () => {
    let { daySelected, yearSelected } = this.state;
    this.initCustomDates();
    await this.carregarVariaveisEstado();
    await this.loadEstruturaAutorizada();
    this.loadMarketingOffers();
    this.loadMarketingVouchers();
    this.handleLoadYearDataCoastCenter(yearSelected);
  ...
  loadIndicatorsMarketing = async (startDate, finalDate, tipo) => {
    try {
      const { empresaAtiva, usuarioAtivo } = this.state;
      ...
      const url =
        '/v1/dashboard/marketingdata/' + empresaAtiva + '/' + usuarioAtivo;
      let datas = {
        startDate,
        finalDate,
        cidades,
            ...
            result.forEach((_indicator) => {
              ...
              if (tipo === 'custom') {
                ...
                _indicator.priorLabel =
                  ...
                  ' á ' +
                  moment(_indicator.endDatePrior, 'DD/MM/YYYY').format('DD/MM');
  ...
  loadMarketingOffers = async () => {
    try {
      ...
      const url =
        '/v1/dashboard/marketingoffers/' + empresaAtiva + '/' + usuarioAtivo;
      ...
      if (!retorno) {
        ...
        if (retorno.data && retorno.data.sucess) {
          ...
          //console.log('result');
          //console.log(result);
          this.setState({ marketingOffers: result });
        } else {
          return [];
        }
      }
  ...
  loadMarketingVouchers = async () => {
    try {
      const { empresaAtiva, usuarioAtivo } = this.state;

      const url =
        '/v1/dashboard/marketingvouchers/' + empresaAtiva + '/' + usuarioAtivo;
      ...
      if (!retorno) {
        ...
        if (retorno.data && retorno.data.sucess) {
          ...
          //console.log('result');
          //console.log(result);
          this.setState({ marketingVouchers: result });
        } else {
          return [];
        }
      }
  ...
  loadMarketingCoastCenter = async (year) => {
    try {
      const { empresaAtiva, usuarioAtivo } = this.state;

      const url =
        '/v1/dashboard/marketingcoastcenter/' +
        empresaAtiva +
        '/' +
        usuarioAtivo +
        '/' +
      ...
      if (!retorno) {
        ...
        if (retorno.data && retorno.data.sucess) {
          ...
          // //console.log('result');
          //console.log(result);
          this.setState({ marketingCoastCenter: result });
        } else {
          return [];
        }
      }
  ...
  handleLoadDayData = async (daySelected) => {
    ...
    let customerIndicators = await this.loadIndicatorsMarketing(
  ...
  handleLoadYearDataCoastCenter = async (yearSelected) => {
    this.setState({ yearSelected });
    let year = moment(yearSelected).format('YYYY');
    await this.loadMarketingCoastCenter(year);
  };
  ...
  render() {
    const {
      ...
      marketingVouchers,
      yearSelected,
      marketingCoastCenter,
    } = this.state;
    return (
      <>
        <Header />
        <Container>
          <SubContainerTitle>DASHBOARD MARKETING</SubContainerTitle>
          <SubContainer>
          ...
          <SubContainerTitle>OFERTAS VIGENTES</SubContainerTitle>
          <SubContainer>
            {marketingOffers !== undefined && (
              <OffersIndicators {...this.props} data={marketingOffers} />
            )}
          </SubContainer>
          <SubContainerTitle>
            VOUCHERS / NEGOCIAÇÕES INDIVIDUAIS
          </SubContainerTitle>
          <SubContainer>
            {marketingVouchers !== undefined && (
              <VouchersIndicators {...this.props} data={marketingVouchers} />
            )}
          </SubContainer>
          <SubContainerTitle>VERBAS CENTROS DE CUSTOS</SubContainerTitle>
          <SubContainer>
            <LineTopFilter>
              <DataFilter>
                <Icon tiny>autorenew</Icon>
                <DatePicker
                  ...
                  onChange={(date) => this.handleLoadYearDataCoastCenter(date)}
                  locale={ptBR}
                  placeholderText="Selecione um ano"
                  dateFormat="yyyy"
                  showYearPicker
                  customInput={<CustomCalendarInput />}
                />
            ...
            {marketingCoastCenter !== undefined && (
              <CoastCenterIndicators
                {...this.props}
                data={marketingCoastCenter}
              />
            )}
          </SubContainer>
        </Container>
